...
void pok_instrumentation_task_archi(const uint32_t id) {
  ...
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     "
         "<address_space_name>partition%d</address_space_name>\n",
         pok_threads[id].partition);
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     "
         "<address_space_name>partition%d</address_space_name>\n",
         pok_threads[id].partition);
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     <name>task%d</name>\n", id);
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     <capacity>%lld</capacity>\n",
  ...
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     <deadline>%lld</deadline>\n",
  ...
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]     <period>%lld</period>\n",
...
void pok_instrumentation_partition_archi(const uint8_t id) {
  ...
  printf(
      "[INSTRUMENTATION][CHEDDAR][ARCHI]    <cpu_name>pokkernel</cpu_name>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    <text_memory_size> "
         "0</text_memory_size>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    <stack_memory_size> "
         "0</stack_memory_size>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    <data_memory_size> "
         "0</data_memory_size>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    <heap_memory_size> "
         "0</heap_memory_size>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    <scheduler quantum= \"%d\">",
         pok_partitions[id].period);
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI]    "
         "POSIX_1003_HIGHEST_PRIORITY_FIRST_PROTOCOL </scheduler>\n");
  printf("[INSTRUMENTATION][CHEDDAR][ARCHI] </address_space>\n");
}

void pok_instrumentation_running_task(const uint32_t id) {
  printf("[INSTRUMENTATION][CHEDDAR][EVENTS] <running_task>   ");
  printf("%lld", POK_GETTICK());
  printf("  task%d", id);
  printf("</running_task>\n");
}
